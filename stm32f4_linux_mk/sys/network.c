...
//  uint16_t phy_id =  ETH_ReadPHYRegister(DP83848_PHY_ADDRESS, 2);
//  uint16_t phy_number =  ETH_ReadPHYRegister(DP83848_PHY_ADDRESS, 3);

//  os_printf("phy_id:0x%04x", phy_id);
//  os_printf("phy_number:0x%04x", phy_number);

  /* Get Ethernet link status*/
  if(ETH_ReadPHYRegister(DP83848_PHY_ADDRESS, PHY_SR) & PHY_DUPLEX_STATUS)
  {
    EthStatus |= ETH_LINK_FLAG;
  }

  os_printf("EthStatus:0x%02x", EthStatus);
...
{
    os_printf("eth link status:%d", netif->flags);
#if 0
